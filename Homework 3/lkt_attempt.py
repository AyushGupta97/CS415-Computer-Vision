...
lk_params = dict(winSize=(5, 5),  # You have finetune these (originally all 2)
                 maxLevel=2,
    ...
    def run(self):
        ...
        while True:
            ...
            if len(self.tracks) > 0:
                ...
                p0 = np.uint8([tr[-1]  # changed from float32 to uint8
            ...
            if self.frame_idx % self.detect_interval == 0:  # Every few frames we have to recompute features, this
                ...
                mask = np.uint8(frame_gray)  # to fix empty mask error
                mask[:] = 255
                for x, y in [np.uint8(tr[-1]) for tr in self.tracks]:
                ...
                if p is not None:
                    for x, y in np.uint8(p).reshape(-1, 2):
